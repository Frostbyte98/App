...
public class Blocco : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        try
        {
            if (this.tag == "Rossa" || this.tag == "Viola" || this.tag == "Verde")
            {
                daltonicText = this.transform.GetChild(0).GetChild(1).GetComponent<Text>();
            }
            else
            {
                daltonicText = null;
            ...
            if (Schermo.daltonico)
            {
                daltonicText.enabled = true;
            }
            else
            {
                daltonicText.enabled = false;
            }
        }
        catch (System.NullReferenceException e)
        {
            System.Console.WriteLine("Eccezzione: {0}", e);
        }
